...
- (NSData * _Nullable)sm4_encryptData:(NSData *)plainInData withCipherKey:(NSString *)key {
    ...
    unsigned char sm4Key[kCCKeySizeAES128 + 1];
    ...
    //加密-ecb mode
    sm4_crypt_ecb(&ctx, SM4_ENCRYPT, plainInDataLength + p, plainInChar, cipherOutChar);
...
- (NSData * _Nullable)sm4_encryptData:(NSData *)plainInData withCipherKey:(NSString *)key iv:(NSString *)iv{
    if (plainInData == nil) {
        NSLog(@"got an empty data!");
        return plainInData;
    }
    ...
    //定义输出密文的变量
    unsigned char cipherOutChar[plainInDataLength + p];
    const char* ivKey = [iv UTF8String];
    size_t ivLen = strlen(ivKey)+1;
    unsigned char ivPtr[ivLen];
    memcpy(ivPtr, ivKey, ivLen);
    ...
    sm4_crypt_cbc(&ctx, SM4_ENCRYPT, plainInDataLength + p, ivPtr, plainInChar, cipherOutChar);
    //testEncodejiami(plainInDataLength + p, plainInChar, cipherOutChar);
    //对加密的数据输出
    NSData *cipherTextData =  [[NSData alloc]initWithBytes:cipherOutChar length:sizeof(cipherOutChar)];
    //    NSLog(@"密文NSData=%@",cipherTextData);
...
- (NSData * _Nullable)sm4_decryptData:(NSData *)cipherData withCipherKey:(NSString *)key {
    ...
    if (key.length > SM4_BLOCK_SIZE) {
        key = [key substringToIndex:SM4_BLOCK_SIZE];
    }
    ...
    unsigned char cipherTextChar[cipherDataLength];
    memcpy(cipherTextChar, cipherData.bytes, cipherDataLength);
    //调用解密方法，输出是明文plainOutChar
    unsigned char plainOutChar[cipherDataLength];
...
//    testDecodejiemi(cipherTextData.length, cipherTextChar, plainOutChar);
    ...
    //ecb mode
    sm4_crypt_ecb(&ctx, SM4_DECRYPT, cipherDataLength, cipherTextChar, plainOutChar);
    
    //由于明文是填充过的，解密时候要去填充，去填充要在解密后才可以，在解密前是去不了的
    int p2 = plainOutChar[sizeof(plainOutChar) - 1];//p2是填充的数据，也是填充的长度
    int outLength = (int)cipherDataLength-p2;//明文的长度
    /*
    //去掉填充得到明文
    ...
    */
    //Modify by jagtu:去掉填充得到明文并且转成NSData
    NSData *outData = [[NSData alloc]initWithBytes:plainOutChar length:outLength];
    
    return outData;
}

- (NSData * _Nullable)sm4_decryptData:(NSData *)cipherData withCipherKey:(NSString *)key iv:(NSString *)iv{
    if (cipherData == nil) {
        NSLog(@"got an empty data!");
        return cipherData;
    }
    ...
    sm4_context ctx;
    sm4_setkey_dec(&ctx,sm4Key);
    sm4_crypt_cbc(&ctx, SM4_DECRYPT, (int)cipherData.length, ivPtr, cipherTextChar, plainOutChar);
...
- (NSData *_Nullable)sm3_hashWithPainData:(NSData *)plainData {
    ...
    UInt8 outputbyte[32]={0};
    sm3((uint8_t*)plainData.bytes, (uint32_t)plainData.length, outputbyte);
    
    return [NSData dataWithBytes:outputbyte length:32];
...
- (BOOL)sm2_verifyWithPlainString:(NSString *)str withSigned:(NSString *)sign withUID:(NSString *)uid withPublicKey:(NSString *)key  withEncyptType:(NSString *)typeString {
    ...
    unsigned long originlen = keyData.length;
    
    if (originlen >= keylen) {
        unsigned char keyDataBytes[keylen];
        memset(keyDataBytes, 0, sizeof(keyDataBytes));
        unsigned long originlen = keyData.length;
        const char* keyBytes = keyData.bytes;
        for (int i = 0; i < keylen; i++) {
            keyDataBytes[i] = keyBytes[i + (originlen - keylen)];
        }
        int ret = GM_SM2VerifySig((unsigned char *)signData.bytes, signData.length, (unsigned char *)srcData, strlen(srcData), (unsigned char *)uidData, uidLen, keyDataBytes, keylen);
        return ret == 0;
    }else{
        return NO;
    }
